...
public class NotificationsManagement {
    ...
    *///
    ...
    public void sendGroupRequest(Group group, String userId) {
        String message = userId + " has requested to join your group";
        for (String username : group.getAdminIds()) {
            Notifications addedPost = new Notifications(message, username, LocalDateTime.now());
            ndb.storeNotifications(addedPost);
        }
    }
